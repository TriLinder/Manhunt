execute as @e[tag=hunted,limit=1,sort=nearest] at @s run setblock ~ 0 ~ lodestone
summon minecraft:item ~ ~ ~ {Item:{id:"minecraft:compass", Count:1b, tag:{display:{Name:'{"text":"Manhunt Compass","bold":true,"color":"aqua"}'}, LodestoneTracked:1b, LodestonePos:{X:0, Y:0, Z:0}, LodestoneDimension:"minecraft:overworld"}}, PickupDelay:100s}
